...
#change contact info
curl -i -X PUT -H "Authorization: Bearer $token" -d PhoneCountry=1 -d Phone=3035551234 -d PostalCode=80203 -d FirstName=John -d LastName=Doe -d Email=johndoe@example.com -d "Address1=200 East Colfax Avenue" -d City=Denver -d Region=CO -d Country=US https://api.namebright.com/rest/account/domains/example.com/contacts/technical
